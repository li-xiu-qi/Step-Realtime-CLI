 ...
 * cli-app.ts — 完整应用逻辑。
 *
 * 由轻量入口 cli.ts 动态加载（--help 不触发本模块），含完整 agent 栈：
 * 全部 import + 模块级初始化 + 所有命令处理（-p / -r / sessions / subagents / ACP / 交互 TUI）。
 *
 * 本文件不得设置 NODE_ENV，也不得 import 任何设置它的模块。
  ...
  const subCtx: ToolContext = {
    ...
    runSubagent: createSubagentRunner({
      ...
      compaction: {
        ...
        blockRatio: config.compaction.blockRatio,
        maxCompactionPerTurn: config.compaction.maxCompactionPerTurn,
        preserveThinking: config.compaction.preserveThinking,
      },
