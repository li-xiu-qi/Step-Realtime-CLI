...
const zh = {
  ...
  'cmd.dream': '睡眠巩固：后台提取对话中的 durable signal 并写入 memory/skill store（--dry-run 预览，--global 跨 session）',
  ...
  'cmd.run': '执行命令并将输出回传进对话上下文：/run <命令>',
  'cmd.rename': '给当前会话改名：/rename（留空清除自定义名）',
  'cmd.agents': '查看当前会话派生的子 agent 会话（只读回看历史）',
  'cmd.handoff': '切换到指定子 agent 会话并追加指令：/handoff <id> [追加指令] · /handoff back（返回上层）',
  'cmd.handoff.busy': 'handoff 只在非 busy 时可用',
  ...
  'cli.subagents.unknownSub': '未知子命令：{sub}（可用：list / show <id> / delete <id> / search <关键词>）',
...
const en: Record<keyof typeof zh, string> = {
  ...
  'cmd.dream': 'Sleep consolidation: extract durable signals from conversation into memory/skill store (--dry-run preview, --global cross-session)',
  ...
  'cmd.run': 'Run a command and inject output into conversation context: /run <command>',
  ...
  'cmd.handoff': 'Switch to a subagent session and append an instruction: /handoff <id> [instruction] · /handoff back (return to previous)',
  'cmd.handoff.busy': 'handoff is only available when not busy',
  ...
  'cli.subagents.unknownSub': 'Unknown subcommand: {sub} (available: list / show <id> / delete <id> / search <query>)',
