 ...
 *  - `fetch`：web_extract 本地提取的完整正文（更完整）
 *
 * web_extract 优先读缓存，命中且未过期则直接返回；未命中或过期才走网络。
 * web_search 每次执行后把结果写入缓存，供后续 web_extract 复用。
 *
 * ## 为什么按字节记账，不只按条目数
 *
 * 早先只有 `maxSize = 100`（条目数）一道上限，而单篇正文的唯一约束是 web_extract 的
