 ...
 *
 * - FOLD_KEEP_RECENT_TURNS：折叠后保留的最近完整轮数。30 是基于日常使用中长会话占比与折叠成本权衡的取值。
 ...
 *   初值 200 过于保守（日常长跑根本到不了，等于空窗，根据实测长会话行数分布下调）：
 *   30+5 保留更多上下文（用户最常回看最近几十轮）的同时，
...
export class PiChat {
   ...
   * 与 notifyQueue 分开是因为两者的消费规则不同：用户输入必须逐条按序发出（显式授权，
   * 合并会打乱语义与权限节奏），通知批量一次发完（系统事实陈述，一次看完全部模型才能
   * 判断该读哪个输出）。优先级上通知更靠前——它是用户上一条输入的产物，模型处理新指令
   * 前必须先看到它；见 planTurnEnd 注释。
   */
  private queue: string[] = [];
  /** 系统注入待发队列（后台任务通知）。优先于用户队列投递：批量一次性全部交给模型。 */
  ...
  private async runTurn(
    ...
    const runSubagent = createSubagentRunner({
      ...
      compaction: {
        ...
        blockRatio: this.deps.config.compaction.blockRatio,
        maxCompactionPerTurn: this.deps.config.compaction.maxCompactionPerTurn,
          preserveThinking: this.deps.config.compaction.preserveThinking,
      },
