 ...
 *
 * 2026-09-06 降回 276：移除 Ctrl+V 贴图入口时删掉中间态提示 app.image.reading（中英各一），
 * 再删掉 Ink 遗留的死 key workingStatus.tokens（pi 版状态栏已改用 StatusLine 的
 * `context: N% (X/Y)` 格式，无渲染路径）。两次各 -1，累计 278 → 276。
 */
