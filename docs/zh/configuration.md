...
| `block_ratio` | 0.93 | trigger_ratio–0.99 | 硬阻塞：达到此值后强制完成压缩才放行回合。必须 >= trigger_ratio |
| `max_compaction_per_turn` | Infinity | 1–20 | 单轮最大自动压缩次数。达到后本轮不再自动压缩，直接放行 |
| `reserved_tokens` | 32000 | 0–500000 | 剩余窗口不足此值即触发压缩 |
| `preserve_thinking` | false | 布尔 | 压缩时保留 thinking/redacted_thinking 块为标记（`[思考]` / `[已编辑思考]`），而非丢弃 |
