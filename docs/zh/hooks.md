...
hooks 让你在不改代码的前提下，于生命周期事件点执行自己的 shell 命令：观察 agent 在做什么，或在特定时机阻断它。本页讲配置形态、六个事件的语义、执行与阻断约定。字段速查见[配置参考](./configuration.md)。
...
- **PreOutput 每轮只生效一次**。它看到的是模型刚产出的正文，exit 2 会阻断本轮并注入原因（作为下一轮用户消息），同一轮内不会二次触发。
