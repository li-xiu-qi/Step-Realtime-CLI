...
>
> **与 pi 版本同步（硬约束，先读规范再动手）**：本仓与 `step-code-pi` 不共享 git 历史，
> 同步规则以产品设计仓 `两仓关系与差异.md` 的「两版本同步规范（权威）」一节为唯一事实源，本文不重复。
> 从本仓视角最易违反的三条：
> 1. **L2 共享层双向、逐字节一致**：改了 `src/tools`/`provider`/`session`/`config`/`skill`/`agent` 等，
>    必须同步到 pi 仓，分别在两仓 CHANGELOG 登记、在同步档案记账。禁止静默单边改。
> 2. **L1 特性对标双向**：本仓新落地一个用户可见特性，要在同步周期内评估 pi 侧「实现 / 明确判不做」并登记。
> 3. **L3 TUI 层不同步代码**：本仓 `src/tui/` 的改动，pi 侧会人工翻译到 `src/tui-pi/`；React 特有的时序/hooks 改动
>    pi 侧往往不需要，不必强求对方照搬。
> 操作细节（路径映射、打 patch 命令、已同步范围）见设计仓 `pi-tui实验/20260816-主仓同步机制与已同步范围.md`。
