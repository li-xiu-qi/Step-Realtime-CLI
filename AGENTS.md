...
| 层级 | 放哪 | 内容 |
|------|------|------|
| 用户文档 | 本仓 `docs/zh/` + `docs/en/` | 功能说明、快捷键、命令参考、配置说明 |
| 产品设计文档 | 设计仓 `step-code-product-design/` | 设计决策、竞品对标、PRD、迭代记录、对内 CHANGELOG |

- 本仓 `AGENTS.md` 本身是源码仓的纪律文件，不放产品设计内容
- 设计仓的路径：`C:\Users\ke\Documents\projects\CodeProjects\Windows-Native\step-code-suite\step-code-product-design\`
- 设计仓内部结构见其 `README.md`，前端设计文档在 `前端设计-pi版/` 目录下
- **写产品/交互/设计类文档时，先确认放哪个仓，不要往源码仓里塞设计文档**

## 参考资料来源

调研竞品 / 对照源码前，先查参考源码资产清单确认本地是否已有 clone——本地没有再去网上拉，拉到后登记进去：

- **参考源码资产清单**（唯一权威来源）：设计仓「外部参考与对比」目录下的参考源码资产清单——登记全部竞品 CLI、上游原仓库、SDK/库的本地路径、许可证、可借鉴范围、最后同步状态。源码实体集中在 `CodeProjects/_reference/`。
- **借鉴点与差距分析**：同目录的竞品横向调研与借鉴清单
- **专项调研**：设计仓「外部参考与对比」下按日期编号的对照文档

> 做跨项目开发时，如果任务涉及"看看竞品怎么做"或"找某项目源码"，第一步读参考源码资产清单，不要凭记忆拼路径。
